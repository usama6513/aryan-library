...
def remove_book(index):
    if 0 <= index < len (st.session.library):
...
# Search Books
def search_books(search_item , search_by):
    search_term = search_term.lower()
    results = []
    
    
    for book in st.session.library :
    ...
    if stats ['total_books'] > 0:
        ...
        if stats['genres']:
                ...
                if stats['decades']:
                    ...
                    # Load Library
load_library()
st.sidebar.markdown("<h1 style= 'text-align: center;'> Nvigation</h1>" , unsafe_allow_html= True)
lottie_book = load_lottieurl("https://assests.lottieflies.com/temp/1f20_aKAfIn.json")
if lottie_book:
    with st.sidebar:
        ...
        with col1:
            ...
            with col2:
                genre = st.selectbox("Genre" , [
                    "Friction" , "Non-Friction" , "Science" , "Romance" , "Technology" , "Fanstasy" , "Poetry" , "Self-Help" , "Art" , "Religion" , "History" , "Other"
                ])
                ...
                for i , book in enumerate(st.session_state.library):
                    with cols[i % 2]:
                        st.markdown(f"""<div class = 'book-card'> 
                                    ...
                                    <p><strong>Publlication Year:</strong> {book['publication year']}</p>
...
st.markdown("----")
st.markdown("Copyright @ 2025 Fatimah Noman Personal Library Manager" , unsafe_allow_html=True)
